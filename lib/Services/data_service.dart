...
class DataService {
  ...
  static Future<Map<String, dynamic>> toggleFollow({
    // required int userId,
    required bool isFollowing, // true means already following, so unfollow
  }) async {
    final prefs = await SharedPreferences.getInstance();
    String? token = prefs.getString('token');
    print('token ${token}');
    final url = isFollowing
        ? "https://eventgo-live.com/api/v1/user/51/unfollow"
        : "https://eventgo-live.com/api/v1/user/51/follow";
